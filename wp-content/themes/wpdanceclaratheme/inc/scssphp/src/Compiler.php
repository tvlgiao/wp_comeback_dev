...
class Compiler
{
    ...
    public $rootEnv; /* Giao: change protected to public */
    protected $rootBlock;
    ...
    public function compile($code, $path = null)
    {
        ...
        // tvlgiao: to pass Theme Check// $this->stderr         = fopen('php://stderr', 'w');
        
        // \Bench::reset(); // giao debug
        // \Bench::start(); // giao debug
        $this->parser = $this->parserFactory($path);
        // \Bench::mark('parserFactory'); // giao debug
        $tree = $this->parser->parse($code);
        // \Bench::mark('parse'); // giao debug
        $this->parser = null;

        $this->formatter = new $this->formatter();
        $this->rootBlock = null;
        $this->rootEnv   = $this->pushEnv($tree);
        // \Bench::mark('pushEnv'); // giao debug

        $this->injectVariables($this->registeredVars);
        // \Bench::mark('injectVariables'); // giao debug
        $this->compileRoot($tree);
        // \Bench::mark('compileRoot'); // giao debug
        $this->popEnv();
        // \Bench::mark('popEnv'); // giao debug

        $out = $this->formatter->format($this->scope);
        // \Bench::mark('format'); // giao debug
        // \Bench::stop(); // giao debug
        // file_put_contents('bench.log', print_r(array('STATISTICS'=>\Bench::getStats(), 'MARKS'=>\Bench::getMarks(), 'ERRORS'=>\Bench::getErrors()), true), FILE_APPEND); // giao debug
    ...
    protected function compileChild($child, OutputBlock $out)
    {
        ...
        switch ($child[0]) {
            ...
            case Type::T_ASSIGN:
                ...
                // $compiledValue = json_encode($value1, JSON_PRETTY_PRINT); // giao debg
            ...
            case Type::T_WARN:
                ...
                $line = $this->sourceLine;
                $value = $this->compileValue($this->reduce($value, true));
                error_log("Line $line WARN: $value\n");
                break;
    ...
    protected function importFile($path, $out)
    {
        ...
        if (isset($this->importCache[$realPath])) {
            ...
            // tvlgiao: use wp_filesystem to pass Theme Check
            $fs = \wpdanceclaratheme\Helper\wp_filesystem();

            $code   = $fs->get_contents($path);
