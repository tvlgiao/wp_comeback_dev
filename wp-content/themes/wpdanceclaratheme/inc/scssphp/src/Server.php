...
class Server
{
    ...
    protected function needsCompile($out, &$etag)
    {
        ...
        if (is_readable($metadataName)) {
            ...
            $metadata = unserialize($fs->get_contents($metadataName));
    ...
    protected function compile($in, $out)
    {
        ...
        $css     = $this->scss->compile($fs->get_contents($in), $in);
    ...
    public function compileFile($in, $out = null)
    {
        ...
        $compiled = $this->scss->compile($fs->get_contents($in), $in);

        if ($out !== null) {
            // tvlgiao: use wp_filesystem to pass Theme Check
            $fs = wpdanceclaratheme\Helper\wp_filesystem();

            return $fs->put_contents($out, $compiled);
        }
